...
- **`flow-preflight.sh` is the single source of truth** for the machine-checkable rules — `git-state`, `resolved` (the deferral `DONE`-gate), `wellformed` (deferral front-matter shape), `spec-path` (id → detail-file path, every shape/name/location), `autonomy` (resolves `checkpoint`/`auto-build` by precedence), and `rubric-basis`/`rubric-drift` (fingerprint + drift-check the persisted validation rubric). The guards, `/flow:run`, `/flow:validate`, `/flow:lint`, and `/flow:ship` call it, so a rule is defined once and can't drift.
- **Descriptive spec filenames (1.17).** A detail file is named `<id>-<slug>.md` — the slug inferred from the index Title (local) or `System.Title` (ado) — so the backlog reads clearly in a file tree, a `git log --stat`, or a PR's changed-files list. **The front-matter `id:` is the identity and is never parsed back out of the filename**, so a dashed id (`BL-12`) stays unambiguous. Bare `<id>.md` is permanently valid (hooks accept both, knob-free); `.flow/config.yml`'s `flow.spec_filename: id` opts a project out, and `/flow:lint --rename` is the opt-in retrofit. **Never resolve an id to a path by hand — call `spec-path`.**
...
- **Validation done-gate (opt-in per spec).** A spec may carry a `validate:` front-matter block (`target`/`intent` + optional `lens`/`design_system`). When present, `/flow:run`'s done-step dispatches the 1.14 `flow-ux-validator` agent directly (one per lens, **serial**) and triages findings before `DONE` — a finding is fixed here or re-homed via the deferral protocol; `NOT APPLICABLE` passes cleanly. No block ⇒ no-op. It's a prompt-level gate (in `skills/run/reference/implement.md` §4), not a hook — the spec-guard doesn't allowlist front-matter keys, so `validate:` needs no guard change.
- **Persisted project rubric (opt-in, engineer-owned).** The design-system layer of the validation rubric lives in the target repo at `.flow/validate/{ui,ux}.md` (committed, like `.flow/config.yml`); baseline heuristics stay in `skills/validate/reference/`. The `flow-ux-validator` **reads** it (never writes — read-only re the project tree); the **main thread** (the `/flow:validate` skill) persists it confirm-first — bootstrap a source-inferred draft on first run, offer a refresh when it drifts. Drift is a fingerprint check over the rubric's `basis:` files via `flow-preflight.sh rubric-drift`/`rubric-basis`. The done-gate only reads + nudges to `/flow:validate`; it never authors the rubric.
...
└── specs/               # One detail file per spec, named <id>-<slug> (flat, or a dir = orchestrator + .T<n>-<ctx> task files); specs/archive/ for DONE/SUPERSEDED
```
