...
- **`flow-preflight.sh` is the single source of truth** for the machine-checkable rules — `git-state`, `resolved` (the deferral `DONE`-gate), `wellformed` (deferral front-matter shape), `autonomy` (resolves `checkpoint`/`auto-build` by precedence), and `rubric-basis`/`rubric-drift` (fingerprint + drift-check the persisted validation rubric). The guards, `/flow:run`, `/flow:validate`, `/flow:lint`, and `/flow:ship` call it, so a rule is defined once and can't drift.
