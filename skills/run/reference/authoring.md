...
- **Slugify:** lowercase; every run of non-`[a-z0-9]` becomes a single `-`; trim leading/trailing `-`; truncate **at a word boundary** to **40 chars** (a task's context suffix: **20**). If the result is empty (a title of pure punctuation), use the bare id.
